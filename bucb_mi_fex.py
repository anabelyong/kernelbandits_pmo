...
def bayesian_optimization_bucb_mi(
    ...
    for r in range(n_rounds):
        bo_loop_logger.info(f"\n--- Trial {trial_id}, Round {r} (train size={len(gp._smiles_train)}) ---")
        t0 = time.time()
        ...
        batch = build_batch_bucb_mi(gp, params, pool, fps, K=B, sqrt_beta=sqrt_beta, eta_t=eta_t, rho_floor=rho_floor)
        ...
        for j, (smi, f_true, mu_j, std_j, ucb_j) in enumerate(
            ...
            bo_loop_logger.info(f"[round {r}] pick {j+1}/{B}: {smi} | UCB+MI={ucb_j:.4g} | f={f_true:.4g}")
