...
def bayesian_optimization_bucb(
    ...
    n_rounds=20,
    batch_size=5,
    beta=1.0,            # <-- pass beta directly
    rho_floor=None,
    trial_id=1
):
    # convert beta -> sqrt_beta
    sqrt_beta = np.sqrt(beta)

    # === Logging setup (per trial + beta) ===
    log_dir = f"logs_bucb/trial_{trial_id}/beta_{beta:.2f}"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, "logs_terminal_output_jax_bucb.log")
    csv_file = os.path.join(log_dir, "logs_terminal_output_jax_bucb.csv")
    ...
    bo_loop_logger.info(
        f"=== Starting BUCB Bayesian Optimization Trial {trial_id} (beta={beta:.2f}, sqrt_beta={sqrt_beta:.2f}) ==="
    )
    ...
    for r in range(n_rounds):
        bo_loop_logger.info(
            f"\n--- Trial {trial_id}, beta={beta:.2f}, Round {r} (train size={len(gp._smiles_train)}) ---"
        )
        t0 = time.time()
        ...
        for j, (smi, f_true, mu_j, std_j, ucb_j) in enumerate(
            ...
            bo_loop_logger.info(f"[round {r}] pick {j+1}/{B}: {smi} | UCB={ucb_j:.4g} | f={f_true:.4g}")
            ...
            rows.append({
                ...
                "mean_pred": mu_j,
                "std_pred": std_j,
                "train_size": len(gp._smiles_train),
                "round_time_s": time.time() - t0
            })

        # Periodic save
        if r % 2 == 0 or r == n_rounds - 1:
    ...
    log_fh.close()
    print(f"[Trial {trial_id}] Completed beta={beta:.2f}. Logs at {log_dir}")
    ...
    for trial_id in range(1, NUM_TRIALS + 1):
        ...
        for beta in BETA_VALUES:
            bayesian_optimization_bucb(
                ...
                n_rounds=20,
                batch_size=5,
                beta=beta,              # <-- pass beta, not sqrt_beta
                rho_floor=None,
                trial_id=trial_id
            )
